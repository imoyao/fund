...
TABLES_LIST_PERCENTAGE = [TABLE_STOCK_PERCENTAGE, TABLES_INDEX_PERCENTAGE, TABLE_HYDIRD_PERCENTAGE,
                          TABLE_BOND_PERCENTAGE,
                          TABLE_BOND_DINGKAI_PERCENTAGE, TABLE_FEEDER_PERCENTAGE, TABLE_TIERED_LEVERAGED_PERCENTAGE,
                          TABLE_QDII_PERCENTAGE]
...
def get_rise_by_code(fundcode, table_name, start_date, end_date):
    sql = "select %s, %s, %s from %s where FundCode = '%s' and Date >= '%s' and Date <= '%s'" % (
    DATE, RANGETODAY, RANKTODAY, table_name, fundcode, start_date, end_date)
    with conn:
        try:
            cur = conn.cursor()
            cur.execute(sql)
        ...
        return (fundcode, float(format(range_totol, '.4f')) * 100, riseCount, downCount, range_max, range_min,
                rank_totol / length)
